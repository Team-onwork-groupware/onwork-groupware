...
`OnWork`는 Spring Boot 3.3(Java 21) + React/Vite(TypeScript) + PostgreSQL 16 + Redis 7 기반 그룹웨어다.
이 저장소는 `남재현교수님_프로젝트/dev_automation_agent_templates`의 DevAuto 멀티에이전트 파이프라인으로 개발·검증한다.
...
이번 0529 명세 정합화 작업은 사용자가 승인한 계획 범위에 포함되므로, `schema.sql`/JPA 엔티티/API 경로/프론트 호출 변경은 `plan.md`에 기록한 범위 안에서 자동 진행한다. 단, 새 외부 의존성이나 secret은 여전히 사용자 확인이 필요하다.
