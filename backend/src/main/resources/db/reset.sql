...
-- 개발팀원(강태양,6)·영업팀원(전지후,14)이 연차 신청(PENDING) → 팀장(최현준·정수연) 부재이므로
-- 대행자 경영지원팀장 박지수(3) 결재함에 '대행'으로 바로 표시됨(동적 결재자 산정).
INSERT INTO leave_requests (user_id, leave_balance_id, start_date, end_date, days_used, reason, status)
SELECT 6,  id, CURRENT_DATE + 7, CURRENT_DATE + 8, 2.0, '연차 신청 (개인 사유)', 'PENDING'
  FROM leave_balances WHERE user_id=6  AND leave_type_id=1 AND year=2026;
INSERT INTO leave_requests (user_id, leave_balance_id, start_date, end_date, days_used, reason, status)
SELECT 14, id, CURRENT_DATE + 7, CURRENT_DATE + 8, 2.0, '연차 신청 (개인 사유)', 'PENDING'
  FROM leave_balances WHERE user_id=14 AND leave_type_id=1 AND year=2026;

-- 대행 결재자(박지수, id 3)에게 결재 대기 알림
INSERT INTO notifications (user_id, type, ref_type, ref_id, message)
SELECT 3, 'LEAVE_REQUESTED', 'LEAVE', lr.id, '새 휴가 신청이 결재 대기 중입니다 (대행)'
  FROM leave_requests lr WHERE lr.status='PENDING' AND lr.user_id IN (6, 14);
